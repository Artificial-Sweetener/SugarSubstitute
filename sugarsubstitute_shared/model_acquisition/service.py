...
class ModelAcquisitionService:
    ...
    def acquire(
        ...
        *,
        ...
        existing = self._matching_existing(destination, expected_hash)
    ...
    @staticmethod
    def _matching_existing(destination: Path, sha256: str) -> Path | None:
        """Return an already-owned SafeTensor with the exact requested hash."""

        for candidate in destination.glob("*.safetensors"):
            if candidate.is_file() and not candidate.is_symlink():
                if _file_sha256(candidate) == sha256:
                    return candidate
        return None
