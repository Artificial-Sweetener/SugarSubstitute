...
_RANDOM_SEED_ICON = AppIcon.GAME_DIE_HIGH_CONTRAST
_FIXED_SEED_ICON = AppIcon.LOCKED_HIGH_CONTRAST


def _fixed_policy() -> QSizePolicy.Policy:
    """Return the fixed sizing policy used by seed child controls."""

    return QSizePolicy.Policy.Fixed


def _maximum_policy() -> QSizePolicy.Policy:
    """Return the shrinkable horizontal policy used by the seed control."""

    return QSizePolicy.Policy.Maximum


def _set_strong_focus_policy(widget: QWidget) -> None:
    """Disable focus-by-wheel for one seed control surface."""
...
def _rendered_seed_action(menu: object, action_id: str) -> QAction:
...
class SeedBox(QWidget):
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        if event.key() in {Qt.Key.Key_Up, Qt.Key.Key_Down}:
            ...
            delta = self._step if event.key() == Qt.Key.Key_Up else -self._step
