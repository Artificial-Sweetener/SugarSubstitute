...
_BOX_LABELS: dict[str, ApplicationMessage] = {
    "x": app_text("X coordinate"),
    "y": app_text("Y coordinate"),
    "width": app_text("Width"),
    "height": app_text("Height"),
}


class _BoundingBoxDialog(LocalizedMessageBoxBase):
    ...
    def __init__(self, value: Mapping[str, int], parent: QWidget) -> None:
        ...
        self.title_label = LocalizedSubtitleLabel(app_text("Bounding box"), self)
        self.controls: dict[str, SpinBox] = {}
        grid = QGridLayout()
        grid.setHorizontalSpacing(12)
        grid.setVerticalSpacing(8)
        for index, key in enumerate(_BOX_KEYS):
            label = LocalizedCaptionLabel(_box_label(key), self)
...
class BoundingBoxField(LocalizedPushButton):
...
def _box_label(key: str) -> ApplicationMessage:
    """Return the app-owned visible label for one stable coordinate key."""
