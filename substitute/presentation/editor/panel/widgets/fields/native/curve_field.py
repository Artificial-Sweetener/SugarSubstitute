...
class _CurveDialog(MessageBoxBase):  # type: ignore[misc]
    ...
    def __init__(self, value: object, parent: QWidget) -> None:
        ...
        super().__init__(parent)
        title = SubtitleLabel(self.tr("Curve"), self)
        instructions = CaptionLabel(
            self.tr("Click to add, drag to move, and right-click to remove a point."),
            self,
        )
        self.canvas = CurveCanvas(value, self)
        reset_button = PushButton(self.tr("Reset"), self)
...
class CurveField(PushButton):  # type: ignore[misc]
    ...
    def _refresh_text(self) -> None:
        ...
        self.setText(self.tr("Edit curve ({count} points)").format(count=point_count))
