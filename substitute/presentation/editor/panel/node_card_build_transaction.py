...
class NodeCardBuildTransaction:
    ...
    def commit(self) -> None:
        ...
        try:
            ...
            log_warning_exception(
                _LOGGER,
                "Rolled back node-card field registrations after commit failure",
                error=error,
                cube_alias=self._cube_alias or "",
    ...
    def discard(self, root: QWidget | None) -> NodeCardRegistrationCleanup:
        ...
        try:
            ...
            log_debug(
                _LOGGER,
                "Skipped destruction for an already deleted node-card surface",
                cube_alias=self._cube_alias or "",
                node_name=self._node_name,
                error=str(error),
            )
