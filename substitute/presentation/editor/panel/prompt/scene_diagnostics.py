...
_LOGGER = get_logger("presentation.editor.panel.prompt.scene_diagnostics")
...
class EditorPanelPromptSceneDiagnosticsHost(Protocol):
    ...
    def current_behavior_snapshot(self) -> EditorBehaviorSnapshot | None:
        """Return the current behavior snapshot for scene analysis."""
...
class EditorPanelPromptSceneDiagnosticsController:
    ...
    def _current_behavior_snapshot(self) -> EditorBehaviorSnapshot | None:
        """Return the latest behavior snapshot from the explicit host port."""

        return self._host.current_behavior_snapshot()
