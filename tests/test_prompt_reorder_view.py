    ...
    return PromptReorderProjectionPaintSnapshot(
        ...
        fragments=(
            PromptReorderTextPaintFragment(
                ...
                text_rect=QRectF(left + 4.0, 4.0, 30.0, 14.0),
...
def test_chip_widget_state_maps_direct_overlay_properties_and_cursor() -> None:
    """Widget state construction should leave mutation to the overlay caller."""

    visual_style = _style()
    pressed = prompt_reorder_chip_widget_state(
        3,
        visual_style=visual_style,
    ...
    hovered = prompt_reorder_chip_widget_state(
        4,
        visual_style=visual_style,
        dragged_segment_index=None,
...
def test_chip_widget_states_preserve_segment_index_order() -> None:
    """Widget-state batches should follow the overlay-owned chip order."""

    states = prompt_reorder_chip_widget_states(
        (4, 2),
        visual_style=_style(),
...
def test_reorder_view_is_editor_backed_overlay_paint_surface() -> None:
    """Reorder animation paints through a parent-backed shape mask."""
...
def test_reorder_view_paint_clips_to_owned_region_without_background_fill(
    ...
    assert not view.mask().isEmpty()
    assert view.mask().boundingRect().width() < view.width()
