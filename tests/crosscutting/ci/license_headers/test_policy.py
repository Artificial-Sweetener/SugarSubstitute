...
def test_rewrite_repairs_exact_incomplete_owned_header() -> None:
    """Canonicalize a split file that retained only the owned header prefix."""
    ...
    assert "GNU General Public License for more details." in updated
    assert updated.endswith('\n\n"""Module."""\n')


def test_rewrite_repairs_exact_incomplete_owned_header_at_end_of_file() -> None:
    """Canonicalize an empty package retaining only the owned header prefix."""
    ...
    assert "GNU General Public License for more details." in updated
    assert updated.count("SugarSubstitute - The desktop native Qt front-end") == 1
    assert updated.endswith("<https://www.gnu.org/licenses/>.\n")
