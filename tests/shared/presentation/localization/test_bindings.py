...
def test_binding_collection_releases_setter_targets_with_owner() -> None:
    """Owner destruction must release callbacks retained for retranslation."""

    _application()
    owner = QWidget()
    sink = _TextSink()
    sink_reference = ref(sink)
    bindings = LocalizationBindings(owner)
    bindings.bind_setter(sink.set_text, lambda: "Search")
    del sink
    assert sink_reference() is not None
...
def test_property_binding_releases_explicit_setter_target_with_owner() -> None:
    """Localized property teardown must not retain its setter's target."""

    _application()
    owner = QWidget()
    sink = _TextSink()
    sink_reference = ref(sink)
    set_localized_text(owner, "Search", property_setter=sink.set_text)
    del sink
    assert sink_reference() is not None
...
class _TextSink:
    """Record one translated value without participating in Qt ownership."""

    def __init__(self) -> None:
        """Initialize an empty rendered value."""

        self.text = ""

    def set_text(self, text: str) -> None:
        """Store one rendered application message."""
