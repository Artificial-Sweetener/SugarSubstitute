...
"""Verify manifest-driven app, Fluent, and Qt translation-catalog loading."""
...
def test_english_bundle_uses_source_text_without_translation_delegates(
...
def test_chinese_bundle_loads_app_upstream_fluent_and_qt_catalogs(
    qt_application_owner: QApplication,
) -> None:
...
def test_japanese_bundle_loads_app_owned_widget_text_and_qt_catalogs(
    qt_application_owner: QApplication,
) -> None:
...
def test_korean_bundle_loads_app_owned_widget_text_and_qt_catalogs(
    qt_application_owner: QApplication,
) -> None:
    """Load complete Korean app and Qt delegates with Korean font fallbacks."""

    application = qt_application_owner
    loader = _application_loader(application)
    resolved = resolve_locale(
        LanguagePreference.explicit("ko"),
        ui_languages=("en-US",),
    )
    ...
    assert len(bundle.translators) == 2
    assert bundle.application_font.families()[0] == "Malgun Gothic"
    app_catalog, qt_catalog = bundle.translators
    assert app_catalog.translate("LanguageSelector", "Language") == "언어"
    assert app_catalog.translate("SwitchButton", "On") == "켜짐"
    assert qt_catalog.translate("QPlatformTheme", "Cancel") == "취소"


def test_spanish_bundle_loads_app_upstream_fluent_and_qt_catalogs(
    qt_application_owner: QApplication,
) -> None:
...
def test_missing_required_catalog_fails_before_bundle_commit(
    qt_application_owner: QApplication,
    tmp_path: Path,
) -> None:
