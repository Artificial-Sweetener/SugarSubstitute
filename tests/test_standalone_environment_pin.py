...
def test_default_pin_covers_every_supported_standalone_variant() -> None:
    """Production pinning must not fall back to live data for any variant."""
...
def test_pin_rejects_missing_variant() -> None:
    """A partial upstream catalog must never become a production pin."""
...
def test_pin_rejects_insecure_artifact_url() -> None:
    """Pinned downloads must remain authenticated by HTTPS and checksum."""
...
def test_pin_rejects_malformed_sha256() -> None:
    """Malformed integrity metadata must fail before any managed download."""
...
def test_pin_writer_round_trips_deterministically(tmp_path: Path) -> None:
    """Automation should produce stable reviewable pin changes."""
