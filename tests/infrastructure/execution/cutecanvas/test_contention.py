...
def test_interactive_canvas_work_overtakes_queued_background_work() -> None:
    ...
    try:
        ...
        background_completed = Event()
        background: ExecutionHandle[None, object] = scope.submit(
            ExecutionRequest(
        ...
        background.add_done_callback(lambda _outcome: background_completed.set())
        scope.submit(
            ExecutionRequest(
        ...
        assert background_completed.wait(timeout=1.0)
