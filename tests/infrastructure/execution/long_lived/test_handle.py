...
def test_long_lived_task_stop_cancels_and_dispatches_terminal_outcome() -> None:
    ...
    def work(source: CancellationSource) -> str:
        ...
        started.set()
        if not stop_requested.wait(1.0):
            raise TimeoutError("Close hook did not release long-lived work.")
        assert source.is_cancelled
        return "stopped"

    def close() -> None:
        """Record close ownership and release the worker."""
...
def test_long_lived_task_records_late_stop_before_eventual_completion() -> None:
    ...
    handle.add_done_callback(
        ...
        reason="observe_eventual_completion",
    )
