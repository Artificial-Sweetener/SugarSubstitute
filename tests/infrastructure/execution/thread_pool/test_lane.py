...
def test_thread_pool_lane_cancel_before_start_finishes_cancelled() -> None:
    ...
    try:
        ...
        second_handle.add_done_callback(
            lambda _outcome: None,
            reason="observe_cancellation",
        )
...
def test_thread_pool_lane_releases_settled_handle_without_cyclic_collection() -> None:
    """Release a settled handle as soon as its caller drops ownership."""
    ...
    lane = ThreadPoolExecutionLane(
        ...
        max_workers=1,
        queue_capacity=1,
        thread_name_prefix="release-lane",
    )
    try:
        handle = lane.submit(
            _request(result="done"),
        ...
        handle.add_done_callback(lambda _outcome: None, reason="observe_release")
        dispatcher.wait_for_callbacks(1)
    finally:
        lane.shutdown(wait=True)
