...
def test_real_comfy_process_can_be_discovered_bound_and_closed(tmp_path: Path) -> None:
    ...
    try:
        ...
        observed = _observe_process(process_gateway, process.pid)
...
def _observe_process(
    gateway: PsutilLocalComfyProcessGateway,
    process_id: int,
) -> tuple[LocalComfyProcess, ...]:
    """Observe one owned process through bounded native process-table polling."""

    deadline = time.monotonic() + 5.0
    while time.monotonic() < deadline:
        observed = tuple(item for item in gateway.scan() if item.pid == process_id)
        if observed:
            assert len(observed) == 1
            return observed
        time.sleep(0.05)
    pytest.fail(f"Process {process_id} was not visible before the native timeout")
