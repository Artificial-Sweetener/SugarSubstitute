...
"""Verify workflow-domain model invariants."""
...
def test_cube_state_mutable_defaults_are_instance_isolated() -> None:
    """Keep undo and redo collections isolated per cube instance."""
    first = CubeState(
        cube_id="CubeA",
        version="1.0.0",
        alias="A",
...
def test_workflow_state_runtime_maps_are_instance_isolated() -> None:
    """Keep workflow runtime image and mask maps isolated per workflow."""
    first = WorkflowState()
...
def test_image_meta_keeps_generation_context_fields() -> None:
    """Store the image context used by output routing and labels."""
    meta = ImageMeta(
