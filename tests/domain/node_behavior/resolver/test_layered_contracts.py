...
from tests.support.node_behavior import behavior_payload
...
def _context(
    *,
    node_name: str,
    class_type: str,
    patch: PackageBehaviorPatch | None = None,
) -> NodeBehaviorContext:
...
def test_should_hide_node_overrides_by_class_string() -> None:
...
def test_enabled_switch_precedence() -> None:
...
def test_groups_for_variants() -> None:
...
def test_field_style_for_maps_to_resolved_field_behavior() -> None:
    patch = behavior_payload(
        {
            "controls": {
                "by_field": {
