...
def test_real_shell_alt_arrow_preview_layout_matches_settled_layout(
    ...
    """Keep the held-Alt reorder frame identical to the committed settled layout."""

    field = harness.add_prompt_workflow(initial_text=initial_text)
    harness.set_source_cursor_position(
        field,
    ...
    harness.process_events(cycles=8)
    preview = harness.capture_reorder_rendered_layout(
        field,
        label="alt-arrow-preview",
    )
    ...
    harness.process_events(cycles=8)
    settled = harness.capture_reorder_rendered_layout(
        field,
        label="alt-arrow-settled",
    )
...
def test_real_shell_alt_arrow_keeps_held_chip_border_owned(
    ...
    """Keep one stable active border throughout keyboard-held chip animation."""

    initial_text = "alpha, beta, gamma"
    field = harness.add_prompt_workflow(initial_text=initial_text)
    harness.set_source_cursor_position(field, initial_text.index("beta") + 1)
    editor = field.editor
    harness.focus_editor(field)
    ...
    before_move = harness.capture_reorder_chip_chrome(
        field,
    ...
    def capture_animation_frame(*, reason: str) -> None:
        ...
        if reason == "animation_frame":
            animation_frames.append(
                harness.capture_reorder_chip_chrome(
                    field,
                    segment_index=held_segment_index,
...
def test_real_shell_alt_up_preview_preserves_regional_settled_layout(
    ...
    """Keep a moved chip on the same side of regional chrome as its commit."""
