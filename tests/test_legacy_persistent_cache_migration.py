...
def test_legacy_migration_respects_independently_prepared_cache_owners(
    ...
    production = build_persistent_cache_catalog(source_root=Path(__file__).parents[1])
...
def _prepare(
    ...
    *,
    ...
    catalog = build_persistent_cache_catalog(source_root=Path(__file__).parents[1])
