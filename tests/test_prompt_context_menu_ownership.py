...
"""Tests for direct prompt context-menu preparation and snapshot ownership."""
...
class _Scene:
    ...
    def prepare_position_context(
        ...
        *,
        ...
        """Prepare and return fake source-position state for one menu opening."""

        _ = reason
        return self.prepared_position_context(source_position)
...
def test_context_menu_preparation_and_snapshot_owners_compose_state() -> None:
    """Preparation must precede a passive immutable snapshot read without Qt."""

    lora = _LoraMetadata()
    scene = _Scene()
    segments = _Segments()
    danbooru = _Danbooru()
    controller = PromptContextMenuSnapshotAssembler(
        diagnostics=cast(PromptContextMenuDiagnosticsPort, _Diagnostics()),
        lora_metadata=cast(Any, lora),
        lora_trigger_words=cast(Any, lora),
        scene_publication=scene,
        scene_positions=scene,
        segment_presets=cast(PromptSegmentPresetController, segments),
        danbooru=cast(PromptDanbooruActionController, danbooru),
    ...
    preparation = PromptContextMenuPreparationLifecycle(
        prepare_segment_selection=(
            lambda selected_text, selection_range, read_only, reason: (
                segments.prepare_menu_snapshot_for_selection(
                    selected_text=selected_text,
        ...
        prepare_scene_position=lambda source_position, reason: (
            scene.prepare_position_context(source_position, reason=reason)
        ),
        prewarm_trigger_words=lora.prewarm_prompt,
    )
    ...
    preparation.prepare_opening(source_position=12, reason="test")
...
def test_context_menu_snapshot_records_unavailable_concerns() -> None:
    ...
    controller = PromptContextMenuSnapshotAssembler(
        diagnostics=cast(PromptContextMenuDiagnosticsPort, _Diagnostics(None)),
        lora_metadata=cast(Any, lora),
        lora_trigger_words=cast(Any, lora),
        scene_publication=_Scene(
            scene_key=None,
            queueable_scene_key=None,
            effective_prompt_text="source prompt",
        ),
        scene_positions=_Scene(
            scene_key=None,
            queueable_scene_key=None,
            effective_prompt_text="source prompt",
        ),
