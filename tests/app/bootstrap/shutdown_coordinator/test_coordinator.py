...
"""Cover coordinated shell shutdown orchestration."""
...
def _wait_for(
    ...
    *,
    ...
    """Wait until observable coordinator state reaches a bounded outcome."""
...
class _EventPumpProtocol(Protocol):
    """Describe the Qt application surface retained for wait-call compatibility."""

    def processEvents(self) -> None:
        """Process pending Qt events."""


def _wait_for_cleanup_task_completion(coordinator: ShutdownCoordinator) -> None:
    """Flush the queued late-cleanup callback after its task reaches completion."""

    wait_for_qt_condition(
        lambda: (
            coordinator._cleanup_handle is not None
            and coordinator._cleanup_handle.is_finished
        )
    )
    wait_for_queued_qt_turn()
...
def test_shutdown_coordinator_times_out_cleanup_and_ignores_late_success(
    ...
    _wait_for_cleanup_task_completion(coordinator)
