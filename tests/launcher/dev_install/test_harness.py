...
def test_default_dev_install_root_stays_inside_repo_tmp() -> None:
    """Keep developer proofs away from a maintainer's manual install target."""
...
def test_clean_install_root_refuses_non_default_without_override(
    ...
    """Refuse cleanup outside the disposable default target without an override."""
...
def test_clean_install_root_deletes_named_override_target(tmp_path: Path) -> None:
    """Permit an explicit override only for the named disposable target."""
...
def test_clean_install_root_refuses_bad_override_name(tmp_path: Path) -> None:
    """Reject broad or ambiguous path names even with the explicit override."""
...
def test_dev_install_cli_requires_explicit_non_default_clean_override() -> None:
    """Make the CI cleanup override explicit and independently parseable."""
...
def test_onboarding_probe_uses_offscreen_qt_on_headless_targets(
    ...
    """Configure headless Windows and Linux onboarding probes for offscreen Qt."""
...
def test_onboarding_probe_uses_native_cocoa_on_macos() -> None:
    """Leave macOS onboarding probes on Cocoa for frameless-window support."""
