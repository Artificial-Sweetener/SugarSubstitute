...
def test_preflight_scan_does_not_block_qt_owner_thread() -> None:
    ...
    try:
        ...
        wait_for_qt_condition(lambda: bool(owner_responsive_while_scan_blocked))

        assert owner_responsive_while_scan_blocked == [True]
        assert snapshots == []

        service.release.set()
        wait_for_qt_condition(lambda: bool(snapshots))
...
def test_leaving_page_suppresses_in_flight_monitor_result() -> None:
    ...
    try:
        coordinator.start_preflight()
        assert service.entered.wait(timeout=1.0)
        delivered_before_release = execution_runtime.delivered_completion_count
        coordinator.stop_monitoring()
        service.release.set()
        wait_for_qt_condition(
            lambda: (
                execution_runtime.delivered_completion_count > delivered_before_release
            )
        )
