...
"""Verify CanvasIoService path, metadata, and image-boundary orchestration."""
...
class _ImageLoader(Protocol):
    """Describe the image-loading callback used by the repository double."""

    def __call__(self, path: Path) -> object | None:
        """Load the payload associated with one path."""


class _ImageSaver(Protocol):
    """Describe the image-saving callback used by the repository double."""

    def __call__(self, path: Path, *, image: object) -> bool:
        """Save one image payload at its destination path."""


class _ImageDimensionsReader(Protocol):
    """Describe the dimension-reading callback used by the repository double."""

    def __call__(self, path: Path) -> tuple[int, int] | None:
        """Return dimensions for a readable image path."""


def _load_no_image(path: Path) -> object | None:
    """Return no image for the default repository double."""

    del path
    return None


def _save_image_successfully(path: Path, *, image: object) -> bool:
    """Accept one image operation in the default repository double."""

    del path, image
    return True


def _read_no_dimensions(path: Path) -> tuple[int, int] | None:
    """Return no dimensions for the default repository double."""
...
class _ImageRepositoryDouble:
    """Provide typed image persistence behavior for CanvasIoService tests."""

    load_image_callback: _ImageLoader = _load_no_image
    save_image_callback: _ImageSaver = _save_image_successfully
    image_dimensions_callback: _ImageDimensionsReader = _read_no_dimensions

    def load_image(self, path: Path) -> object | None:
        """Load one image through the configured callback."""

        return self.load_image_callback(path)

    def save_image(self, path: Path, *, image: object) -> bool:
        """Save one image through the configured callback."""

        return self.save_image_callback(path, image=image)

    def save_blank_mask(self, path: Path, *, size: object) -> bool:
        """Accept one blank-mask request in the focused service tests."""

        del path, size
        return True

    def save_blank_image(self, path: Path, *, width: int, height: int) -> bool:
        """Accept one blank-image request in the focused service tests."""

        del path, width, height
        return True
    ...
    def save_resampled_mask(
        ...
        *,
        ...
        """Accept one resampled-mask request in the focused service tests."""

        del source, destination, width, height
        return True

    def image_dimensions(self, path: Path) -> tuple[int, int] | None:
        """Read image dimensions through the configured callback."""

        return self.image_dimensions_callback(path)


def _repo(
    *,
    load_image: _ImageLoader = _load_no_image,
    save_image: _ImageSaver = _save_image_successfully,
    image_dimensions: _ImageDimensionsReader = _read_no_dimensions,
) -> _ImageRepositoryDouble:
    """Build a typed image repository test double."""

    return _ImageRepositoryDouble(
        load_image_callback=load_image,
        save_image_callback=save_image,
        image_dimensions_callback=image_dimensions,
    )


class _ExternalImagesOpener(Protocol):
    """Describe the multi-image operation used by the editor gateway double."""

    def __call__(self, *, images: Sequence[tuple[object, ImageMeta]]) -> bool:
        """Open the supplied images and report whether the operation succeeded."""


@dataclass(frozen=True)
class _ExternalImageGatewayDouble:
    """Provide the external-editor boundary required by one delegation test."""

    open_images_callback: _ExternalImagesOpener

    def open_image(self, *, image: object, image_meta: ImageMeta) -> bool:
        """Accept a single-image request that is outside this test's contract."""

        del image, image_meta
        return True

    def open_images(self, *, images: Sequence[tuple[object, ImageMeta]]) -> bool:
        """Delegate the multi-image request to the configured callback."""

        return self.open_images_callback(images=images)
...
def test_open_images_in_external_editor_delegates_to_gateway() -> None:
    ...
    calls: list[Sequence[tuple[object, ImageMeta]]] = []

    def _open_images(*, images: Sequence[tuple[object, ImageMeta]]) -> bool:
        calls.append(images)
        return True

    gateway = _ExternalImageGatewayDouble(open_images_callback=_open_images)
...
def test_load_recipe_preview_image_delegates_to_image_repository() -> None:
    ...
    def _load_image(path: Path) -> object:
        """Record and return the configured preview image."""

        calls.append(path)
        return preview

    service = CanvasIoService(image_repository=_repo(load_image=_load_image))
