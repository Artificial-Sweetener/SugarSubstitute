...
from substitute.application.prompt_editor.document import cache as cache_module
...
def test_prompt_document_view_cache_evicts_the_oldest_entry_at_its_capacity() -> None:
    """Bound projected document reuse without retaining the oldest snapshot."""

    clear_prompt_document_caches()
    service = PromptDocumentService()
    first_view = service.build_document_view("document cache 0")

    for index in range(cache_module._DOCUMENT_VIEW_CACHE_LIMIT):  # noqa: SLF001
        service.build_document_view(f"document cache {index + 1}")

    assert (
        len(cache_module._DOCUMENT_VIEW_CACHE)
        == cache_module._DOCUMENT_VIEW_CACHE_LIMIT
    )  # noqa: SLF001
    assert cached_prompt_document_view("document cache 0") is None
    assert all(
        cached_view is not first_view
        for cached_view in cache_module._DOCUMENT_VIEW_CACHE.values()  # noqa: SLF001
    )
