...
"""Verify authored-bypass VAE activation through the behavior service."""
...
def _vae_cube(
    *,
    ...
    """Build one VAE-loader cube with optional editor metadata."""

    node: dict[str, object] = {"class_type": "VAELoader", "inputs": {}}
    if mode is not None:
        node["mode"] = mode
...
def test_vae_loader_without_authored_bypass_is_visible_and_enabled() -> None:
    """Keep an ordinary VAE loader visible and active by default."""
...
def test_string_mode_four_does_not_apply_authored_bypass_policy() -> None:
    """Treat only integer mode four as authored-bypass metadata."""
...
def test_bypass_authored_vae_is_hidden_and_disabled_by_default() -> None:
    """Start authored-bypass VAE loaders hidden and inactive in every cube."""
...
def test_revealing_bypass_authored_vae_makes_it_visible_but_disabled() -> None:
    """Show a revealed authored-bypass VAE loader without activating it."""
...
def test_revealed_bypass_authored_vae_can_be_enabled_explicitly() -> None:
    """Require explicit reveal and enable before activating an authored bypass."""
...
def test_enabled_bypass_authored_vae_without_reveal_stays_hidden_and_disabled() -> None:
    """Prevent an enabled-but-hidden authored-bypass VAE state."""
