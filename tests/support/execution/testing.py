...
class RecordingDispatcher:
    ...
    def __init__(self) -> None:
        ...
        self._publication_condition = Condition()
    ...
    def wait_for_callbacks(
        ...
        *,
        ...
        """Wait for worker publication through a notified completion barrier."""

        if expected_count < 1:
            raise ValueError("expected_count must be positive.")
        with self._publication_condition:
            published = self._publication_condition.wait_for(
                lambda: len(self._callbacks) >= expected_count,
                timeout=timeout_seconds,
            )
        assert published, (
            f"Expected {expected_count} callbacks, observed {len(self.callbacks)}."
        )
