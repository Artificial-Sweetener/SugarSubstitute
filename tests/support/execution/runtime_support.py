...
"""Provide shared test helpers for runtime-backed execution adapters."""
...
class _ObservedCompletionDispatcher:
    """Record owner-thread delivery after delegating each completion callback."""

    def __init__(
        self,
        *,
        delegate: TestCompletionDispatcher,
        record_delivery: Callable[[], None],
    ) -> None:
        """Store the real dispatcher and delivery observer."""

        self._delegate = delegate
        self._record_delivery = record_delivery

    def publish(self, callback: Callable[[], None], *, reason: str) -> None:
        """Publish the callback and record only completed owner-thread delivery."""

        def deliver_and_record() -> None:
            """Run the real completion before exposing its delivery barrier."""

            callback()
            self._record_delivery()

        self._delegate.publish(deliver_and_record, reason=reason)
...
class ExecutionRuntimeStub:
    ...
    def __init__(self) -> None:
        ...
        self.closed_count = 0
        self.delivered_completion_count = 0
    ...
    def submitter(
        ...
        *,
        ...
        observed_dispatcher = _ObservedCompletionDispatcher(
            delegate=dispatcher,
            record_delivery=self._record_completion_delivery,
        )
        lane = ThreadPoolExecutionLane(
            ...
            dispatcher=observed_dispatcher,
        )

        is_closed = False

        def close() -> None:
            """Join the test-owned lane and record runtime ownership release."""
            ...
            try:
                ...
                self._close_callbacks.remove(close)
            self.closed_count += 1

        self._close_callbacks.append(close)
        return _RuntimeSubmitter(lane=lane, close_callback=close)

    def _record_completion_delivery(self) -> None:
        """Record one completion after its owner-thread callback has returned."""

        self.delivered_completion_count += 1
