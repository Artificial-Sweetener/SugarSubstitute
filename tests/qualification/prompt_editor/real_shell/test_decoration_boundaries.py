...
@pytest.fixture
def harness(tmp_path: Path) -> Iterator[PromptEditorRealShellScenario]:
    """Create and close the real-shell harness for each boundary scenario."""

    shell_harness = PromptEditorRealShellScenario(artifact_root=tmp_path)
    try:
        yield shell_harness
    finally:
        shell_harness.close()
...
def test_space_insertion_stays_at_each_decoration_boundary(
    ...
    route = harness.input.press_key(field, Qt.Key.Key_Space, text=" ")
    after = harness.snapshots.capture(
        field,
...
def test_additional_tag_remains_inside_weighted_emphasis(
    ...
    harness.input.press_key(field, Qt.Key.Key_Comma, text=",")
    settled_after_comma = harness.snapshots.capture(
        field,
        label="emphasis-after-comma",
    )
    harness.input.press_key(field, Qt.Key.Key_Space, text=" ")
    harness.input.type_text(field, "red eyes")
    completed = harness.snapshots.capture(
        field,
    ...
    assert not snapshot_invariant_violations(completed)
...
def _insert_space_through_path(
    ...
    if insertion_path == "clipboard":
        harness.input.paste_text(field, " ")
        return
