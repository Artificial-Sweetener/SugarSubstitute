...
"""Tests for shared regional color Qt adaptation."""
...
import substitute.presentation.regional.color_provider as color_provider
...
def test_input_mask_color_returns_theme_color_for_first_mask(
    ...
    monkeypatch.setattr(color_provider, "themeColor", lambda: base_color)

    assert color_provider.region_color(0, 4) == base_color
...
def test_input_mask_color_adapts_domain_hue_to_vivid_qcolor(
    ...
    hue, saturation, value, _alpha = cast(
        ...
        color_provider.region_color(1, 2).getHsv(),
    )
