...
"""Verify wheel intent for managed text asset prompt editors."""
...
class _ManualClock:
    ...
    def __init__(self) -> None:
        """Start at the beginning of one isolated interaction."""
    ...
    def advance(self, milliseconds: int) -> None:
        """Advance controlled time without sleeping the test process."""

        self._milliseconds += milliseconds


class _FrameOwner:
    """Create wheel frames and destroy their native Qt state synchronously."""

    def __init__(self) -> None:
        """Track every frame created by one test."""

        self._frames: list[NumberedPromptEditorFrame] = []
    ...
    ) -> tuple[NumberedPromptEditorFrame, _ManualClock]:
        """Create one frame with an independently controlled intent clock."""

        clock = _ManualClock()
        frame = _frame(wheel_adjustment_mode, wheel_intent_now_ms=clock)
        self._frames.append(frame)
        return frame, clock

    def destroy_all(self) -> None:
        """Destroy owned frames in reverse construction order."""
...
@pytest.fixture
def frame_owner() -> Generator[_FrameOwner, None, None]:
    """Own every mounted frame for one wheel interaction test."""
...
def test_numbered_prompt_editor_emphasis_wheel_requires_dwell(
    frame_owner: _FrameOwner,
) -> None:
    """Managed prompt emphasis wheel edits should require token dwell."""

    app = ensure_qapp()
    frame, clock = frame_owner.create()
...
def test_numbered_prompt_editor_numeric_wildcard_wheel_requires_dwell(
    frame_owner: _FrameOwner,
) -> None:
...
def test_numbered_prompt_editor_scroll_still_works_without_token_mutation(
    frame_owner: _FrameOwner,
) -> None:
    """Focused managed prompts should scroll without authorizing token wheel edits."""

    app = ensure_qapp()
    frame, _clock = frame_owner.create()
...
def test_numbered_prompt_editor_focus_required_blocks_hover_dwell_emphasis_wheel(
    ...
    app = ensure_qapp()
    frame, clock = frame_owner.create(PromptWheelAdjustmentMode.FOCUS_REQUIRED)
    frame.resize(520, 300)
    frame.show()
    frame.setPlainText("(cat:1.20)")
    process_events(app, cycles=8)
...
def test_numbered_prompt_editor_focus_required_allows_clicked_emphasis_wheel(
    frame_owner: _FrameOwner,
) -> None:
    """Managed prompt emphasis wheel edits should work after token click activation."""

    app = ensure_qapp()
    frame, _clock = frame_owner.create(PromptWheelAdjustmentMode.FOCUS_REQUIRED)
    frame.resize(520, 300)
    frame.show()
    frame.setPlainText("(cat:1.20)")
    process_events(app, cycles=8)
