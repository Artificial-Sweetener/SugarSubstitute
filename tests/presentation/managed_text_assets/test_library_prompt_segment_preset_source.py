...
"""Test global library prompt-segment preset presentation."""
...
def test_library_prompt_segment_source_rejects_non_global_save_scope() -> None:
    """Library prompt segments should never persist into a workflow-specific scope."""

    source = LibraryPromptSegmentPresetSource(
        UserPresetService(_MemoryUserPresetRepository())
    )
    non_global_scope = PresetSaveScope(
        title="Workflow",
        full_label="Workflow: Test",
    ...
    with pytest.raises(ValueError, match="only the Global scope"):
        source.save_prompt_segment(
            label="Portrait",
            text="detailed portrait, studio lighting",
            scope=non_global_scope,
        )

    assert source.list_prompt_segment_presets().menu_model.sections == ()
