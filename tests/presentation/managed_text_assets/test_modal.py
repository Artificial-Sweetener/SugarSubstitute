...
"""Verify the reusable managed text asset modal."""
...
class _MountedModalOwner:
    """Own modal and parent widgets through synchronous native teardown."""

    def __init__(self) -> None:
        """Track Qt owners in construction order."""

        self._objects: list[QObject] = []

    def create_parent(self) -> QWidget:
        """Create and retain one top-level modal owner widget."""
    ...
    def create_modal(
        ...
        *,
        ...
        """Create and retain one managed text asset modal."""

        modal = _modal(service, parent=parent, error_presenter=error_presenter)
        self._objects.append(modal)
        return modal

    def destroy_all(self) -> None:
        """Destroy children before their retained top-level owners."""

        for candidate in reversed(self._objects):
            destroy_qt_object(candidate)
        self._objects.clear()


@pytest.fixture
def modal_owner() -> Generator[_MountedModalOwner, None, None]:
    """Own every mounted object created by one modal test."""

    owner = _MountedModalOwner()
    yield owner
    owner.destroy_all()
...
def test_managed_text_asset_modal_loads_assets_and_preserves_switched_edits(
    modal_owner: _MountedModalOwner,
) -> None:
    """Switching assets should preserve unsaved editor text in memory."""

    app = ensure_qapp()
    service = _FakeManagedTextAssetService()
    modal = modal_owner.create_modal(service)
    modal.show()
    process_events(app)
...
def test_managed_text_asset_modal_switching_assets_resets_editor_undo_history(
    modal_owner: _MountedModalOwner,
) -> None:
...
def test_managed_text_asset_modal_save_current_clears_dirty_state(
    modal_owner: _MountedModalOwner,
) -> None:
...
def test_managed_text_asset_modal_rows_are_text_only(
    modal_owner: _MountedModalOwner,
) -> None:
...
def test_managed_text_asset_modal_apply_saves_text_only(
    modal_owner: _MountedModalOwner,
) -> None:
...
def test_managed_text_asset_modal_gives_editor_more_width(
    modal_owner: _MountedModalOwner,
) -> None:
...
def test_managed_text_asset_modal_shadow_does_not_filter_editor_descendants(
    modal_owner: _MountedModalOwner,
) -> None:
    """High-frequency editor paints should not invalidate a subtree effect."""

    ensure_qapp()
    modal = modal_owner.create_modal(_FakeManagedTextAssetService())
...
def test_managed_text_asset_modal_uses_owner_window_height(
    modal_owner: _MountedModalOwner,
) -> None:
    """The modal content should take 90 percent of the top-level owner height."""

    app = ensure_qapp()
    owner = modal_owner.create_parent()
    owner.resize(1200, 800)
    owner.show()
    process_events(app)
    service = _FakeManagedTextAssetService()
    modal = modal_owner.create_modal(service, parent=owner)
    modal.show()
    process_events(app)
...
def test_managed_text_asset_modal_reports_operation_errors_through_presenter(
    modal_owner: _MountedModalOwner,
) -> None:
    ...
    modal = modal_owner.create_modal(
        service,
