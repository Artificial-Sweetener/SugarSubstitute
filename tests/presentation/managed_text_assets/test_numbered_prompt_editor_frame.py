...
"""Verify the managed text asset numbered prompt editor frame."""
...
@pytest.fixture
def frame() -> Generator[NumberedPromptEditorFrame, None, None]:
    """Own one numbered editor frame through synchronous native teardown."""

    owned_frame = _frame()
    yield owned_frame
    destroy_qt_object(owned_frame)


def test_numbered_prompt_editor_frame_delegates_text_and_counts_lines(
...
def test_numbered_prompt_editor_frame_expands_gutter_after_99_lines(
    frame: NumberedPromptEditorFrame,
) -> None:
...
def test_numbered_prompt_editor_frame_uses_projection_source_line_geometry(
    frame: NumberedPromptEditorFrame,
) -> None:
...
def test_numbered_prompt_editor_frame_counts_trailing_empty_source_line(
    frame: NumberedPromptEditorFrame,
) -> None:
