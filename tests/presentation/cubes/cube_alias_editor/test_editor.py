...
def _editor(text: str = "SDXL/Text to Image") -> CubeAliasEditor:
    ...
    editor.begin(text)
    wait_for_qt_condition(editor.isVisible)
    return editor


@pytest.fixture(autouse=True)
def _clean_up_top_level_alias_editors() -> Iterator[None]:
    """Dispose each test's application-filtered alias editor deterministically."""

    yield

    for widget in _app().topLevelWidgets():
        if isinstance(widget, CubeAliasEditor):
            widget.cancel()
