...
"""Verify MetaRegistry link-widget cleanup, refresh, and command routing."""
...
from .support import ComboDouble as _Combo
from .support import ParentWidgetDouble as _ParentWidget
...
def test_update_link_widgets_passes_all_buffers_layout_and_beautifier(
    ...
    assert all_buffers["A"] == panel._cube_states["A"].buffer
    assert call[5] == "layout-object"
    assert callable(call[6])
    assert cast(Callable[..., object], call[6]).__name__ == "beautify_label"
    assert kwargs["node_definition_gateway"] is panel.node_definition_gateway


def test_prompt_node_link_refresh_passes_width_groups_by_identity(
    monkeypatch: MonkeyPatch,
) -> None:
...
def test_node_link_refresh_passes_width_groups_by_identity(
    monkeypatch: MonkeyPatch,
) -> None:
...
def test_node_link_refresh_creates_missing_eligible_selector_from_title_surface(
    ...
    def _setup_node_link(*args: object, **_kwargs: object) -> tuple[_Combo, str]:
        endpoint = cast(NodeLinkEndpoint, args[2])
        title_layout = args[5]
        combo = _Combo(parent_obj=object(), parent_widget_obj=_ParentWidget(layout))
        widget_map = cast(dict[tuple[str, object], _Combo], args[1])
        widget_map[(endpoint.cube_alias, endpoint.identity)] = combo
...
def test_sampler_scheduler_refreshes_do_not_receive_shared_width_labels(
    ...
    sampler_field_state = cast(ChoiceLinkFieldState, sampler_kwargs[0]["field_state"])
    scheduler_field_state = cast(
        ChoiceLinkFieldState,
        scheduler_kwargs[0]["field_state"],
    )
    assert sampler_field_state.literal_options == ("euler", "heun")
    assert scheduler_field_state.literal_options == (
        "normal",
        "karras",
    )
