...
def _activate_section_layout(section: QWidget) -> None:
    """Resolve owned layout and field-width work without draining Qt events."""

    section.ensurePolished()
    layout = section.layout()
    if layout is not None:
        layout.activate()
    sync_width_group = getattr(section, "sync_string_line_edit_width_group", None)
    if callable(sync_width_group):
        sync_width_group()
...
def test_cube_section_finalization_removes_provisional_masonry_overlap() -> None:
    ...
    try:
        ...
        section.show()
        _activate_section_layout(section)
