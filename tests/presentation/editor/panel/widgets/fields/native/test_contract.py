...
"""Verify native Comfy field controls, localization, and rendering."""
...
def _current_fluent_tooltip_filter_type() -> type[object]:
    """Return the current Fluent tooltip class after test-module reloads."""

    module = importlib.import_module(
        "sugarsubstitute_shared.presentation.fluent_tooltips"
    )
    return cast(type[object], getattr(module, "FluentToolTipFilter"))
...
def test_native_fields_retranslate_in_place_without_losing_values() -> None:
    ...
    try:
        ...
        assert isinstance(
            ...
            _current_fluent_tooltip_filter_type(),
        )
        assert box.text() == "x 1・y 2・3×4"
...
_JAPANESE_NATIVE_TRANSLATIONS: Mapping[str, str] = MappingProxyType(
    {
