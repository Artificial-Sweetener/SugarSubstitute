...
def test_projection_surface_scheduled_metadata_failure_remains_retryable(
    ...
    monkeypatch.setattr(surface._presentation_runtime.rebuild, "rebuild", fail_rebuild)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        surface._presentation_runtime.rebuild, "rebuild", original_rebuild_projection
    )
    monkeypatch.undo()
    set_surface_prompt_state(surface, document_view, metadata_render_plan)
