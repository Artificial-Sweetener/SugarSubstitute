...
def reveal_wildcard_controls(
    ...
    _send_viewport_mouse_move(box, reset_point)
...
def _send_viewport_mouse_move(box: PromptEditor, point: QPoint) -> None:
    """Deliver one native viewport move with an explicit offscreen global position."""

    global_point = box.viewport().mapToGlobal(point)
...
def test_prompt_editor_wildcard_projection_hover_updates_without_breaking_typing_flow(
    ...
    _send_viewport_mouse_move(box, token_rect_for(box, token).center().toPoint())
    wait_for_qt_condition(
        lambda: (
            (hovered_token := surface_for(box).hovered_token()) is not None
            and hovered_token.source_start == token.source_start
            and hovered_token.source_end == token.source_end
        )
    )
...
def test_prompt_editor_wildcard_tokens_coexist_with_emphasis_controls(
    ...
    _send_viewport_mouse_move(box, anchor_rect.center().toPoint())
    token_weight_controls_for(box).refresh_geometry()
    wait_for_qt_condition(
        lambda: (
            (visible_token := token_weight_controls_for(box).visible_token) is not None
            and visible_token.token_id == emphasis_token.token_id
        )
    )

    visible_token = token_weight_controls_for(box).visible_token
