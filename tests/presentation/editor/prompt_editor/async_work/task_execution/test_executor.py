...
"""Test the prompt-editor local task-executor adapter."""
...
from tests.support.execution.runtime_support import prompt_task_executor
...
def test_task_executor_can_cancel_queued_work_before_it_runs() -> None:
    ...
    release_blocker.set()
    assert blocker_completed.wait(3.0)
    assert blocking_handle.is_finished is True
    executor.shutdown()
