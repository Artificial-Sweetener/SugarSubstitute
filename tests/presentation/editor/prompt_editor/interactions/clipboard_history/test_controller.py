...
"""Test prompt-editor clipboard-history routing."""
...
from substitute.presentation.editor.prompt_editor.interactions.clipboard_history_controller import (
...
@dataclass(slots=True)
class _Sink:
    """Record cursor effects and published edit commits."""
    ...
    def apply_edit_commit(self, commit: PromptEditCommit[str]) -> None:
        """Record one published source transaction."""
...
def _harness(
    ...
    *,
    ...
    edit_execution.set_pending_key_flusher(flusher)
...
def test_copy_writes_selected_source_text_without_mutation() -> None:
    ...
    assert harness.edit_execution.session.source_text == "alpha beta"
    assert harness.sink.commits == []
...
def test_cut_flushes_before_selection_command_and_requests_deletion() -> None:
    ...
    assert harness.clipboard.writes == ["alpha"]
    assert harness.edit_execution.session.source_text == " beta"
    assert harness.sink.commits[-1].source_edit is not None
    assert harness.sink.commits[-1].source_edit.start == 0
    assert harness.sink.commits[-1].source_edit.end == 5
...
def test_literal_paste_requests_normalized_source_replacement() -> None:
    ...
    assert harness.edit_execution.session.source_text == "alpha gamma"
    assert harness.sink.commits[-1].origin is PromptSourceEditOrigin.PASTE
    assert harness.paste_completions == ["paste"]
...
def test_undo_and_redo_restore_results_reach_sink() -> None:
    ...
    assert [
        commit.next_snapshot.source_text for commit in harness.sink.commits[-2:]
    ] == [
        "alpha",
        "alpha beta",
    ]
