...
"""Contracts for prompt-editor LoRA picker popup presentation."""
...
class _PresenterFactory:
    """Own presenters and their test-created Qt parent widgets."""

    def __init__(self) -> None:
        """Initialize an empty test-local parent registry."""
    ...
    def create(
        ...
        *,
    ...
    ) -> PresenterBuildResult:
        """Build one presenter with explicit test-local Qt ownership."""
        ...
        return presenter, data_source, popup_factory, executor

    def close(self) -> None:
        """Dispose every parent owned by the current test."""
...
@pytest.fixture
def presenter_factory() -> Iterator[_PresenterFactory]:
    """Provide a test-local presenter owner with deterministic teardown."""

    factory = _PresenterFactory()
    yield factory
    factory.close()
...
def test_lora_picker_presenter_does_not_open_when_feature_is_disabled(
    presenter_factory: _PresenterFactory,
) -> None:
    """Disabled picker state should suppress popup creation."""

    presenter, data_source, factory, _ = presenter_factory.create(ready=False)
...
def test_lora_picker_presenter_opens_with_prepared_rows_and_menu_position(
    presenter_factory: _PresenterFactory,
) -> None:
    ...
    presenter, data_source, factory, _ = presenter_factory.create(
        items=(item,),
...
def test_lora_picker_presenter_replaces_existing_popup(
    presenter_factory: _PresenterFactory,
) -> None:
    """Opening a second picker should retire the previous popup."""

    presenter, _, factory, _ = presenter_factory.create(items=(_item(),))
...
def test_lora_picker_presenter_uses_cursor_position_without_menu_position(
    presenter_factory: _PresenterFactory,
) -> None:
    """Opening without a context-menu anchor should use the cursor position."""

    cursor_pos = QPoint(90, 123)
    presenter, _, factory, _ = presenter_factory.create(
        items=(_item(),),
        menu_position=None,
        cursor_position=cursor_pos,
    )
...
def test_lora_picker_presenter_refreshes_visible_popup_rows(
    presenter_factory: _PresenterFactory,
) -> None:
    ...
    presenter, data_source, factory, _ = presenter_factory.create(items=(first,))
    presenter.open_lora_picker()
...
def test_lora_picker_presenter_uses_empty_rows_for_cold_snapshot(
    presenter_factory: _PresenterFactory,
) -> None:
...
def test_lora_picker_presenter_ignores_missing_or_hidden_popup_refresh(
    presenter_factory: _PresenterFactory,
) -> None:
    """Refresh should be a no-op when no visible popup is owned."""

    presenter, data_source, factory, _ = presenter_factory.create(items=(_item(),))
...
def test_lora_picker_presenter_inserts_selected_lora_schedule_text(
    presenter_factory: _PresenterFactory,
) -> None:
    ...
    presenter, data_source, factory, executor = presenter_factory.create(items=(item,))
    presenter.open_lora_picker()
...
def test_lora_picker_presenter_ignores_invalid_activation_payload(
    presenter_factory: _PresenterFactory,
) -> None:
    """Only catalog-item activations should insert text."""

    presenter, data_source, factory, executor = presenter_factory.create(
        items=(_item(),)
    )
    presenter.open_lora_picker()
