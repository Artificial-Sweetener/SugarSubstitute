...
def build_motion_fixture() -> tuple[
    ...
    host.show()
    wait_for_qt_condition(lambda: host.isVisible() and content_body.height() > 0)
...
def test_accordion_motion_replaces_in_flight_transition_safely() -> None:
    ...
    try:
        ...
        wait_for_qt_condition(
            lambda: -state.expanded_height < controller.content_offset_y() < 0
        )
        controller.toggle()
        wait_for_qt_condition(lambda: not state.animating)
...
def test_accordion_motion_does_not_reflow_owner_on_each_animation_frame() -> None:
    ...
    try:
        ...
        controller.toggle()
        wait_for_qt_condition(
            lambda: -expanded_height < controller.content_offset_y() < 0
        )
...
def test_accordion_collapsed_body_does_not_reserve_parent_layout_spacing() -> None:
    ...
    host.show()
    wait_for_qt_condition(lambda: host.isVisible() and card.height() > 0)
    try:
        controller.toggle()
        wait_for_qt_condition(lambda: not state.animating)
