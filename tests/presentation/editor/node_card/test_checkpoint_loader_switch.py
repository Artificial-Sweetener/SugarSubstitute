...
class DummyCubeState:
    def __init__(
        ...
        """Store one minimal cube state for node-card behavior proof."""
...
class DummyPanel(QWidget):
    def __init__(
        ...
        cube_states: dict[str, DummyCubeState],
    ) -> None:
        """Store the panel state consumed by the node-card builder."""
    ...
    def is_connection(self, val: object) -> bool:
        """Report that this focused case contains no connection nodes."""

        return False


def ensure_qapp() -> QApplication:
    """Return the test process QApplication owner."""
...
def test_checkpoint_loader_switch_policy_and_default_enabled() -> None:
    ...
    nodes_a: dict[str, dict[str, object]] = {
        node_name: {"class_type": node_type, "inputs": {}}
    }
    nodes_b: dict[str, dict[str, object]] = {
        node_name: {"class_type": node_type, "inputs": {}, "mode": 4}
    }
