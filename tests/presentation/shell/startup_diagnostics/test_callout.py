...
def callout_qt_application() -> Iterator[QApplication]:
    """Keep one worker-local Qt application alive for callout tests."""
...
def _place_cursor_over(anchor: QWidget) -> None:
    """Place the native cursor on the visible anchor before QFluent tip creation."""

    QCursor.setPos(anchor.mapToGlobal(anchor.rect().center()))
