...
def test_resolve_workflow_name_uses_trimmed_tab_label() -> None:
    """Resolve workflow names from current tab text after trimming whitespace."""
...
def test_resolve_workflow_name_falls_back_when_tab_missing_or_blank() -> None:
    """Use the stable untitled fallback for absent and blank workflow tabs."""
...
def test_mark_input_canvas_changed_marks_canvas_and_generation_surfaces() -> None:
    """Mark all canvas and generation surfaces dirty after content mutations."""
...
def test_mark_input_canvas_presentation_changed_excludes_generation() -> None:
    """Exclude generation surfaces when only canvas presentation changes."""
