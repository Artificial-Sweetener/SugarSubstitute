...
"""Verify cube-stack composition for workflow surfaces."""
...
def test_create_cube_stack_configures_stack_and_shell_signals(
    ...
    """Preserve tab policy and shell signal wiring."""
    created_stacks: list[FakeCubeStack] = []

    def cube_stack_factory(parent: object) -> FakeCubeStack:
        """Create and record one cube stack."""
        stack = FakeCubeStack(parent)
    ...
    fake_stack = cast(FakeCubeStack, stack)
...
def test_cube_stack_signal_handlers_are_intentional_noops() -> None:
    """Keep interim tab signals side-effect free."""
    shell = build_workflow_shell()
    factory = WorkflowUiFactory(shell)
