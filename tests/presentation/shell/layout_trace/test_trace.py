...
def test_log_editor_width_trace_collects_live_shell_layout_context(
    ...
    """Record live shell layout fields alongside caller-supplied context."""
    ...
    shell = SimpleNamespace(
        ...
        canvas_host_container=SimpleNamespace(width=lambda: 260),
    ...
    assert records == [
        {
            ...
            "canvas_host_container_width": 260,
...
def test_safe_trace_helpers_tolerate_missing_or_invalid_widgets() -> None:
    """Fail closed when tracing torn-down or malformed widget collaborators."""

    class _BrokenWidth:
        def width(self) -> int:
            """Raise the representative deleted-widget error."""

            raise RuntimeError("widget deleted")

    class _BrokenSplitter:
        def sizes(self) -> list[int]:
            """Raise the representative deleted-splitter error."""
