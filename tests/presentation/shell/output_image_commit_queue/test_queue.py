...
def test_commit_queue_prioritizes_failed_preparation_within_batch_bound() -> None:
    """Failed preparation should be delivered before later prepared output work."""
    ...
    def project(_workflow_id: str, _image_id: object = None) -> None:
        """Ignore projection requests while testing queue ordering."""
    ...
    def commit(prepared: PreparedOutputImage) -> OutputImageRegistrationResult | None:
        committed.append(prepared.request.node_id)
        return None

    queue = PreparedOutputCommitQueue(
        commit_prepared=commit,
        handle_failure=lambda failure: failures.append(failure.message),
        projection_scheduler=scheduler,
        batch_size=1,
    )
    queue.enqueue_prepared(_prepared("prepared"))
    queue.enqueue_failed(
        FailedOutputImagePreparation(
            request=_prepared("failed").request,
