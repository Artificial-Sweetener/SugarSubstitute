...
def terminal_view_qt_application() -> Iterator[QApplication]:
    """Keep one process-local Qt application alive for terminal view tests."""
...
def owned_qt_objects() -> Iterator[list[QObject]]:
    """Destroy every native Qt object created by the current test."""
...
def test_terminal_output_view_is_one_surface_without_header_chrome(
...
def test_terminal_output_view_replays_stream_history_and_redraws_in_place(
    owned_qt_objects: list[QObject],
) -> None:
...
def test_terminal_output_view_copy_and_clear_actions_operate_on_bound_stream(
    owned_qt_objects: list[QObject],
) -> None:
...
def test_terminal_output_view_applies_shared_terminal_style(
    owned_qt_objects: list[QObject],
) -> None:
...
def test_terminal_output_view_uses_fixed_pitch_terminal_font(
    owned_qt_objects: list[QObject],
) -> None:
...
def test_terminal_output_view_disables_qfluent_smooth_scrolling(
    owned_qt_objects: list[QObject],
) -> None:
...
def test_terminal_output_view_stays_inside_layout_contents(
    owned_qt_objects: list[QObject],
) -> None:
    """The terminal surface should not overflow a parent card's content rect."""

    parent = QWidget()
    owned_qt_objects.append(parent)
...
def test_terminal_output_view_stays_pinned_to_bottom_for_wrapped_stream_output(
    ...
    wait_for_qt_condition(
        lambda: (
            ...
            and _end_of_document_bottom_gap(view) <= _MAX_BOTTOM_CHROME_GAP_PX
        )
    )
...
def test_terminal_output_view_direct_append_uses_shared_progress_semantics(
    owned_qt_objects: list[QObject],
) -> None:
