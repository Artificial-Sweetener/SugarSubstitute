...
def test_download_review_is_an_editable_thumbnail_cart() -> None:
    ...
    assert cards[0].size().toTuple() == (204, 226)
    assert cards[0].portrait.size().toTuple() == (184, 206)
...
def test_nine_model_checkout_matches_picker_density_and_keeps_totals_in_hero() -> None:
    ...
    assert all(card.size().toTuple() == (204, 226) for card in cards)
    assert all(card.portrait.size().toTuple() == (184, 206) for card in cards)
