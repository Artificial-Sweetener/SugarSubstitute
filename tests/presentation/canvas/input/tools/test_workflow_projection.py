...
class _MountedProjection:
    ...
    def __init__(self) -> None:
        ...
        self.application = ensure_qt_application()
...
def test_queued_refresh_uses_current_workflow_and_close_rejects_late_work() -> None:
    ...
    try:
        QTimer.singleShot(
            0,
            lambda: deliveries.append(mounted.projection.refresh_document_context()),
        )
        mounted.workflow = replacement_workflow
        mounted.profile = _synthetic_profile()
        wait_for_qt_condition(lambda: deliveries == [True])
        assert (
            mounted.runtime.palette.presentation_for(InputCanvasToolId.SMART_MASK)
            is None
        )
        ...
        QTimer.singleShot(
            0,
            lambda: deliveries.append(mounted.projection.refresh_workflow_profile()),
        )
        wait_for_qt_condition(lambda: deliveries == [True, False])
        assert (
            mounted.runtime.palette.presentation_for(InputCanvasToolId.SMART_MASK)
            is None
        )
