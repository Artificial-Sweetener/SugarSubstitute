...
"""Verify live Input node previews through the shared CuteCanvas document."""
...
from PySide6.QtCore import QCoreApplication, QEvent, QPointF, QRect, QRectF, QSize, Qt
...
def test_preview_compact_fit_preserves_portrait_source_aspect(
    input_document_factory: InputDocumentFactory,
) -> None:
...
def test_live_node_previews_share_authority_and_survive_erratic_rebinding(
    ...
    try:
        ...
        wait_for_qt_condition(lambda: preview_changes.count() > 0)
...
def test_large_raster_mask_preview_renders_authoritative_coverage(
    ...
    try:
        ...
        wait_for_qt_condition(lambda: _center_color(preview).value() > 240)
...
def test_live_picker_content_uses_shared_rounded_highlight_surface(
    input_document_factory: InputDocumentFactory,
) -> None:
    ...
    try:
        ...
        for picker in (image_picker, mask_picker):
            ...
            hover_point = surface.rect().center()
            global_hover_point = surface.mapToGlobal(hover_point)
            QApplication.sendEvent(
                surface,
                QEnterEvent(
                    QPointF(hover_point),
                    QPointF(hover_point),
                    QPointF(global_hover_point),
                ),
            )
            after = before

            def hover_frame_changed() -> bool:
                """Observe the picker surface after its hover repaint completes."""

                nonlocal after
                after = surface.grab().toImage()
                return after != before

            wait_for_qt_condition(hover_frame_changed)
            assert after != before
