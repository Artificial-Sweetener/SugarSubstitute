...
"""Verify shared-edge mode temporarily enables only mask-layer movement."""
...
def test_edge_resize_enables_all_masks_and_restores_exact_normal_policies(
...
def test_mounted_input_masks_resize_one_shared_edge_and_export_aligned(
    ...
    wait_for_qt_condition(lambda: document.canvas.activeEditSession() is not None)

    assert committed == []
    assert document.canvas.applyActiveEditSession()
    wait_for_qt_condition(
        lambda: document.canvas.activeEditSession() is None and committed == [None]
    )
    ...
    wait_for_qt_condition(lambda: committed == [None, None])
    policy.close()
    document.close()


def test_policy_restores_every_visited_image_after_context_switch(
    input_document_factory: InputDocumentFactory,
) -> None:
