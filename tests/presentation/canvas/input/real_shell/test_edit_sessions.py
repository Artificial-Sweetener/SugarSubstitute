...
def test_real_shell_shared_edge_session_owns_history_settlement_and_lifecycle(
    ...
    try:
        ...
        wait_for_qt_condition(
            lambda: isinstance(
                harness.input_canvas.contextual_toolbar.page,
                InputEditSessionContextualToolbarPage,
            )
        )
        ...
        wait_for_qt_condition(
            lambda: (
                harness.input_canvas.edit_sessions.snapshot is None
                and tools.palette.active_tool_id == InputCanvasToolId.PAN_ZOOM
            )
        )
        ...
        wait_for_qt_condition(
            lambda: (
                (active := harness.input_canvas.edit_sessions.snapshot) is not None
                and active.undo_depth == 1
                and active.redo_depth == 0
                and (
                    pan_state := tools.palette.presentation_for(
                        InputCanvasToolId.PAN_ZOOM
                    )
                )
                is not None
                and not pan_state.enabled
                and (
                    edge_state := tools.palette.presentation_for(
                        InputCanvasToolId.SHARED_EDGE_RESIZE
                    )
                )
                is not None
                and edge_state.enabled
        ...
        wait_for_qt_condition(
            lambda: (
                (active := harness.input_canvas.edit_sessions.snapshot) is not None
                and active.undo_depth == 0
                and active.redo_depth == 1
            )
        )
        session = harness.input_canvas.edit_sessions.snapshot
        assert session is not None
        ...
        wait_for_qt_condition(lambda: durable_changes.count() == 2)
        ...
        wait_for_qt_condition(
            lambda: harness.input_canvas.edit_sessions.snapshot is not None
        )
        page = harness.input_canvas.contextual_toolbar.page
        assert isinstance(page, InputEditSessionContextualToolbarPage)
        ...
        wait_for_qt_condition(
            lambda: harness.input_canvas.edit_sessions.snapshot is None
        )
        assert harness.input_canvas.edit_sessions.snapshot is None
        assert durable_changes.count() == 2
        assert _scene_mappings(canvas.currentScene()) == original_mappings
