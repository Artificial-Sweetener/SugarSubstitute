...
"""Verify Input canvas surface state and compact tool-overlay integration."""
...
from cutecanvas import EditorTransformTarget, ExecutionRuntime, LayerPolicy
...
def test_input_canvas_destruction_closes_document_before_qt_children(
    ...
    monkeypatch.setenv("SUGAR_SUBSTITUTE_STARTUP_HARNESS_DEFER_INPUT_SAM", "1")
    app = _app()
    host = input_mod.InputCanvas(execution_runtime=execution_runtime)
...
def test_input_tool_strip_overlays_only_its_content_height(
    ...
    monkeypatch.setenv("SUGAR_SUBSTITUTE_STARTUP_HARNESS_DEFER_INPUT_SAM", "1")
    app = _app()
    canvas = input_mod.InputCanvas(execution_runtime=execution_runtime)
...
def test_idle_transform_toolbar_emits_no_qt_paint_diagnostics(
    ...
    monkeypatch.setenv("SUGAR_SUBSTITUTE_STARTUP_HARNESS_DEFER_INPUT_SAM", "1")
    app = _app()
    reduced_motion_property = "substitute.reduce_motion"
    previous_reduced_motion = app.property(reduced_motion_property)
    app.setProperty(reduced_motion_property, True)
    host = input_mod.InputCanvas(execution_runtime=execution_runtime)
    runtime = create_input_canvas_tool_system()
    ...
    try:
        ...
        for _repaint in range(8):
            host.canvas.repaint()
            host.contextual_toolbar.repaint()
            app.processEvents()
        ...
        app.setProperty(reduced_motion_property, previous_reduced_motion)
