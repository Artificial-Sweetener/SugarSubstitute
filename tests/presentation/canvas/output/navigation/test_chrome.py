...
def test_deferred_tabbar_geometry_ignores_destroyed_qt_host() -> None:
    """A queued geometry refresh must not access a destroyed Output canvas host."""
    ...
    _install_navigation_controller(host)
    update_output_tabbar_container(
        host,
        single_shot=lambda _milliseconds, callback: scheduled_callbacks.append(
            callback
        ),
    )
