...
import substitute.presentation.widgets.folder_route as folder_route
...
def test_folder_route_bar_renders_breadcrumb_and_emits_ancestor_route(
    qt_application_owner: QApplication,
) -> None:
    """Breadcrumb clicks should emit the selected ancestor route."""

    app = qt_application_owner
    tree = FolderRouteTree(
        (
...
def test_folder_route_root_breadcrumb_retranslates_without_losing_route(
    qt_application_owner: QApplication,
) -> None:
    ...
    app = qt_application_owner
    resource_root = (
        Path(folder_route.__file__).resolve().parents[2] / "resources" / "i18n"
    )
    chinese = QTranslator()
...
def test_folder_route_bar_renders_child_route_buttons(
    qt_application_owner: QApplication,
) -> None:
...
def test_folder_route_bar_hides_root_breadcrumb_when_children_exist(
    qt_application_owner: QApplication,
) -> None:
...
def test_folder_route_bar_leaf_route_uses_one_breadcrumb_row(
    qt_application_owner: QApplication,
) -> None:
...
def test_folder_route_bar_child_routes_scroll_horizontally_without_wrapping(
    qt_application_owner: QApplication,
) -> None:
