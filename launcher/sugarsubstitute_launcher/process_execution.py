...
"""Own child execution, lifetime selection, and retained startup diagnostics."""
...
class ChildProcess(Protocol):
    """Expose execution control independently of the native process adapter."""

    @property
    def pid(self) -> int:
        """Identify the supervised root."""

    def poll(self) -> int | None:
        """Observe root completion and perform owned cleanup."""

    def wait(self, timeout: float | None = None) -> int:
        """Wait for completion with an optional deadline."""

    def terminate(self) -> None:
        """Stop the owned execution."""

    def kill(self) -> None:
        """Force owned execution to stop."""
...
def spawn_detached_process(
    ...
    *,
    ...
    """Start an independent handoff with retained startup output."""
    ...
    with startup_log_path.open("a", encoding="utf-8", errors="replace") as log_file:
        ...
        try:
            with standard_child_process_dll_search_path():
                if supervised and sys.platform == "win32":
                    from sugarsubstitute_shared.windows_process_family import (
                        WindowsProcessFamily,
                    )

                    process: ChildProcess = WindowsProcessFamily.start(
                        command,
